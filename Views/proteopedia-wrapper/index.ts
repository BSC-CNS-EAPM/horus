  ...
  hetGroups = {
    reset: () => {
      const update = this.state
        .build()
        .delete(StateElements.HetGroupFocusGroup);
    ...
    focusFirst: async (
      ...
      compId = "32";
      const compNumber = 200;

      console.log("Transforms", this.state.transforms.asMutable());

      if (!this.state.transforms.has(StateElements.Assembly)) return;
      await PluginCommands.Camera.Reset(this.plugin, {});

      // const data =
      //   this.plugin.managers.structure.hierarchy.current.structures[1]?.cell.obj
      //     ?.data;
      ...
      console.log("focusFirst", compNumber, options);
...
console.log("Mol* Proteopedia Wrapper loaded");
...
console.log(
  "Wrapper version",
  MolStarProteopediaWrapper.VERSION_MAJOR,
  MolStarProteopediaWrapper.VERSION_MINOR
);
...
function loadAndSnapshot(params) {
  PluginWrapper.load(params).then(() => {
    setTimeout(
      () =>
        (snapshot = PluginWrapper.plugin.state.getSnapshot({
          canvas3d: false /* do not save spinning state */,
        })),
      500
    );
  });
...
addControl("Load Asym Unit", () =>
  loadAndSnapshot({ url: url, format: format, isBinary })
);
...
addControl("Custom Chain Colors", () =>
  PluginWrapper.updateStyle(
    { sequence: { coloring: "proteopedia-custom" } },
    true
  )
);
addControl("Default Chain Colors", () =>
  PluginWrapper.updateStyle({ sequence: {} }, true)
);

addControl("HET Spacefill", () =>
  PluginWrapper.updateStyle({ hetGroups: { kind: "spacefill" } }, true)
);
addControl("HET Ball-and-stick", () =>
  PluginWrapper.updateStyle({ hetGroups: { kind: "ball-and-stick" } }, true)
);

addControl("Hide 3DSNFG", () =>
  PluginWrapper.updateStyle({ snfg3d: { hide: true } }, true)
);
addControl("Show 3DSNFG", () =>
  PluginWrapper.updateStyle({ snfg3d: { hide: false } }, true)
);

addControl("Hide Water", () =>
  PluginWrapper.updateStyle({ water: { hide: true } }, true)
);
addControl("Show Water", () =>
  PluginWrapper.updateStyle({ water: { hide: false } }, true)
);
...
addControl("Play To Start", () =>
  PluginWrapper.animate.modelIndex.onceBackward()
);
addControl("Play Palindrome", () =>
  PluginWrapper.animate.modelIndex.palindrome()
);
...
addControl("Apply Evo Cons Style", () =>
  PluginWrapper.coloring.evolutionaryConservation()
);
addControl("Apply Evo Cons Colors", () =>
  PluginWrapper.coloring.evolutionaryConservation({
    sequence: true,
    het: false,
    keepStyle: true,
...
addHeader("Exp. Data");
addControl("Init", () =>
  PluginWrapper.experimentalData.init($("volume-streaming-wrapper"))
);
