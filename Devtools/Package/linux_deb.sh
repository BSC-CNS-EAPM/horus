...
# Create the copyright file inside dist/linux/DEBIAN
cat > dist/linux/DEBIAN/copyright << EOF
                                Apache License
...
# Check if the target exists before removing the old symlink
if [ -e /usr/local/bin/Horus/Horus ]; then
    echo "Correct installation of Horus found at /usr/local/bin/Horus/Horus"
    if [ -e /usr/bin/horus ]; then
        echo "Removing old symlink"
        rm /usr/bin/horus
    fi
fi

echo "Creating new symlink to Horus as 'horus' in /usr/bin/"
ln -s /usr/local/bin/Horus/Horus /usr/bin/horus

# Copy the Desktop file to /usr/share/applications
cp /usr/local/bin/Horus/horus.desktop /usr/share/applications/
...
if [ -L /usr/bin/horus ]; then
    echo "Removing symlink"
    rm /usr/bin/horus
fi

if [ -d /usr/local/bin/Horus ]; then
    echo "Some files may be left in /usr/local/bin/Horus. For example, plugin configurations."
    echo "Please remove them manually if you want to uninstall completely."
fi
